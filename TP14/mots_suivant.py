...
def compte_mots_suivants(nom_fichier):
    ...
    # TODO
    ...
    couples=get_couples(get_mots(nom_fichier))
    out={}
    for couple in couples:
        if couple[0] in out:
            if couple[1] in out[couple[0]]:
                out[couple[0]][couple[1]]=out[couple[0]][couple[1]]+1
            else:
                out[couple[0]][couple[1]]=1
        else:
            out[couple[0]]={couple[1]:1} 
    for couple in couples:
        if couple[1] not in out.keys():out[couple[1]]={}
    return out        
...
def genere_graphe(suivants):
    ...
    with open("mots-suivants.dot", "w") as fichier_dot:
        ...
        ...
        file=open("mots-suivants.dot","w")
        file.write("digraph {\n")
        for mi in suivants:
            for mij in suivants[mi]:
                file.write(f"{mi} -> {mij} [label = {suivants[mi][mij]}]\n")
        file.write("}")
    file.close()            
...
def get_suivant_aleatoire(mot, suivants):
    ...
    # TODO
    ...
    if suivants[mot]=={}:return choice(list(suivants.keys()))
    n=sum(list(suivants[mot].values()))
    mots,probas=list(suivants[mot].keys()),[x/n for x in list(suivants[mot].values())]
    x=random()
    k=0
    for i in range(len(probas)):
        if k<=x<probas[i]+k:return mots[i]
        k+=probas[i]
