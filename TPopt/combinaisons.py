...
def teste():
    ...
    combin=recupere_combinaisons([0, 1, 2, 3, 4], 3)
    for i in combin:print(i)
    ...
    params=recupere_parametres(sys.argv[1])
    print(params)
    ...
    res=recupere_combinaisons(params[0],params[1])
...
def recupere_parametres(filename):
    ...
    k,L=int(file.readline()),file.read().splitlines()
    file.close()
    return L,k
...
def recupere_combinaisons_2(sequence):
    ...
    for i in range(len(sequence)-1):
        for j in range(i+1,len(sequence)):
            L.append((sequence[i],sequence[j]))
    return L        
...
def recupere_combinaisons(sequence, k):
    ...
    if k==2:return recupere_combinaisons_2(sequence)
    else:
        out=[]
        for i in range(len(sequence)-k+1):
            comb=recupere_combinaisons(sequence[i+1:],k-1)
            for cb in comb:out.append(tuple([sequence[i]]+list(cb)))
        return out    
